...
int wifi_controller_start(void) {
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI(CONFIG_LOG_TAG, "connected to ap SSID: %s", CONFIG_WIFI_SSID);
        return 0;
    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGE(CONFIG_LOG_TAG, "Failed to connect to SSID: %s", CONFIG_WIFI_SSID);
        return 1;
    } else {
        ESP_LOGE(CONFIG_LOG_TAG, "UNEXPECTED EVENT");
        return 2;
    }
}
