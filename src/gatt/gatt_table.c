...
static const uint8_t co2_ccc                       = 0x00;
static const uint8_t temp_ccc                      = 0x00;
static const uint8_t cap_ccc                       = 0x00;
...
{
    ESP_LOGI(GATT_TABLE_TAG, "prepare write, handle = %d, value len = %d", param->write.handle, param->write.len);
    esp_gatt_status_t status = ESP_GATT_OK;
    if (prepare_write_env->prepare_buf == NULL) {
        prepare_write_env->prepare_buf = (uint8_t *)malloc(PREPARE_BUF_MAX_SIZE * sizeof(uint8_t));
        prepare_write_env->prepare_len = 0;
        if (prepare_write_env->prepare_buf == NULL) {
            ESP_LOGE(GATT_TABLE_TAG, "%s, Gatt_server prep no mem", __func__);
    ...
    if (param->write.need_rsp){
        ...
        if (gatt_rsp != NULL){
            ...
            if (response_err != ESP_OK){
               ESP_LOGE(GATT_TABLE_TAG, "Send response error");
            }
            free(gatt_rsp);
        }else{
            ESP_LOGE(GATT_TABLE_TAG, "%s, malloc failed", __func__);
        }
    }
...
void example_exec_write_event_env(prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param){
    if (param->exec_write.exec_write_flag == ESP_GATT_PREP_WRITE_EXEC && prepare_write_env->prepare_buf){
        esp_log_buffer_hex(GATT_TABLE_TAG, prepare_write_env->prepare_buf, prepare_write_env->prepare_len);
    }else{
        ESP_LOGI(GATT_TABLE_TAG,"ESP_GATT_PREP_WRITE_CANCEL");
    }
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if (sensoring_handle_table[IDX_CHAR_CFG_A] == param->write.handle && param->write.len == 2){
                    ...
                    if (descr_value == 0x0001){
                        ...
                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, sensoring_handle_table[IDX_CHAR_VAL_A],
                                                sizeof(notify_data), notify_data, false);
                    }else if (descr_value == 0x0002){
                        ESP_LOGI(GATT_TABLE_TAG, "indicate enable");
                        ...
                        ESP_LOGI(GATT_TABLE_TAG, "notify/indicate disable ");
                    }else{
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            if (param->add_attr_tab.status != ESP_GATT_OK){
                ESP_LOGE(GATT_TABLE_TAG, "create attribute table failed, error code=0x%x", param->add_attr_tab.status);
            }
            else if (param->add_attr_tab.num_handle != SEN_IDX_NB){
                ESP_LOGE(GATT_TABLE_TAG, "create attribute table abnormally, num_handle (%d) \
                        doesn't equal to SEN_IDX_NB(%d)", param->add_attr_tab.num_handle, SEN_IDX_NB);
            }
            else {
                ESP_LOGI(GATT_TABLE_TAG, "create attribute table successfully, the number handle = %d\n",param->add_attr_tab.num_handle);
